...
class Schematic extends AbstractSchematic
{
     ...
     * @throws \Exception
     */
    protected function createTable($table, $settings)
    {
        $addFieldSql = '';
        $indexesSql = '';
        $indexesArray = array();
        ...
        foreach ($settings->fields as $field => $fieldSettings) {
            if (isset($fieldSettings->index) == false) {
                $fieldSettings->index = '';
            }
            if (isset($fieldSettings->autoIncrement) && $fieldSettings->autoIncrement) {
                $fieldSettings->autoIncrement = 'AUTO_INCREMENT';
            } else {
                $fieldSettings->autoIncrement = '';
            }
            if (isset($fieldSettings->null) && $fieldSettings->null) {
                $fieldSettings->null = 'NULL';
            } else {
                $fieldSettings->null = 'NOT NULL';
            }
            if (isset($fieldSettings->unsigned) && $fieldSettings->unsigned) {
                $fieldSettings->unsigned = 'unsigned';
            } else {
                $fieldSettings->unsigned = '';
            }
        ...
        if (count($primaryKeys) > 0) {
            $indexesSql .= 'PRIMARY KEY ('.implode(', ', $primaryKeys).'),';
        }
        if (count($uniqueKeys) > 0) {
            $indexesSql .= 'UNIQUE KEY ('.implode(', ', $uniqueKeys).'),';
        }
        if (count($indexKeys) > 0) {
            $indexesSql .= 'INDEX ('.implode(', ', $indexKeys).'),';
        }
     ...
     * @throws \Exception
     */
    protected function createSqlFile($table, $query)
    {
        if (!is_dir($this->sqlDir)) {
            $newDir = @mkdir($this->sqlDir);

            if (!$newDir) {
                throw new \Exception('Unable to create new SQL directory: '.$this->sqlDir);
            }
        }
        ...
        if (!$sql) {
            throw new \Exception('Unable to create SQL file: '.$file);
        }
     ...
     * @throws \Exception
     */
    protected function updateTable($table, $settings)
    {
        ...
        foreach ($settings->fields as $field => $fieldSettings) {
            if (empty($fieldSettings)) {
                throw new \Exception('Invalid Operation! You have no properties on your field ('.$field.')!');
            }
            if (isset($fieldSettings->index) == false) {
                $fieldSettings->index = '';
            }
            ...
            if ($this->dbAdapter->fieldExists($table, $field) || (isset($fieldSettings->rename) && $this->dbAdapter->fieldExists($table, $fieldSettings->rename))) {
                if (isset($fieldSettings->rename)) {
                    if ($this->dbAdapter->fieldExists($table, $fieldSettings->rename)) {
                        ...
                        if ($previousColumn != '') {
                            $columnOrdering = ' AFTER `'.$previousColumn.'`';
                        }
...
'.$indexesSql.';';
     ...
     * @param $settings
     */
    protected function deleteNonSchemaFields($table, $settings)
    {
        $deleteSql = '';
     ...
     */
    protected function applyForeignKeys()
    {
